...
public class PetRepository implements IRepository<Pet> {
    ...
    public List<Pet> getAll() {
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                ...
                SQLstr = "SELECT Genus_Id, pet_id, Name, Birthday, Commands FROM animals ORDER BY pet_id";
    ...
    public Pet getById(int petId) {
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                ...
                SQLstr = "SELECT Genus_Id, pet_id, Name, Birthday FROM animals WHERE pet_id = ?";//"SELECT GenusId, Id, PetName, Birthday FROM pet_list WHERE Id = ?";
    ...
    public int create(Pet pet) {
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                ...
                SQLstr = "INSERT INTO animals (Name, Birthday, Genus_Id) SELECT ?, ?, ?"; //"INSERT INTO pet_list (PetName, Birthday, GenusId) SELECT ?, ?, (SELECT Id FROM pet_types WHERE Genus_name = ?)";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr);
                prepSt.setString(1, pet.getName());
                prepSt.setDate(2, Date.valueOf(pet.getBirthdayDate())); 
                prepSt.setInt(3, PetType.getTypeId(pet.getPetType()));
    ...
    public void train (int id, String command){
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                String SQLstr1 = "SELECT Commands FROM animals WHERE pet_id = ?";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr1);
                prepSt.setInt(1, id);
                resultSet = prepSt.executeQuery();
                if (resultSet.next()){
                ...
                String SQLstr = "UPDATE animals SET Commands =  ? WHERE pet_id = ?";
    ...
    public List<String> getCommandsById (int petId, int commands_type){   
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                if (commands_type == 1){
                    SQLstr = "SELECT Commands FROM animals WHERE pet_id = ?";
                }
                // else {
    ...
    public int update(Pet pet) {
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                SQLstr = "UPDATE animals SET Name = ?, Birthday = ? WHERE pet_id = ?";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr);
    ...
    public void delete (int id) {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                SQLstr = "DELETE FROM animals WHERE pet_id = ?";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr);
                prepSt.setInt(1,id);
                prepSt.executeUpdate();
            }
    ...
    public static Connection getConnection() throws SQLException, IOException {
        ...
        try (FileInputStream fis = new FileInputStream("Prog/Resources/database.properties")) {
